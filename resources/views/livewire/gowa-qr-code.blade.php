<div class="flex flex-col items-center justify-center p-6 text-center space-y-5"
    ...
    @elseif($errorMessage)
        <div class="p-4 rounded-xl bg-red-50 dark:bg-red-950/50 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 text-sm max-w-sm text-left space-y-2">
            <p class="font-semibold flex items-center gap-1.5 text-red-800 dark:text-red-200">
                <svg class="w-4 h-4 text-red-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
                {{ __('gowa-filament::gowa-filament.notifications.error_occurred') }}
            </p>
            <p class="text-xs font-mono break-all opacity-90 leading-relaxed">{{ $errorMessage }}</p>
    ...
    @else
        <div class="flex flex-col items-center justify-center space-y-3 py-8">
            <svg class="w-8 h-8 animate-spin text-primary-600 dark:text-primary-400" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('gowa-filament::gowa-filament.qr.waiting') }}</p>
        </div>
    @endif
</div>
